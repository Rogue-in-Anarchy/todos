...
describe("SetterGetter", function () {
  ...
  async function deploySetterGetter() {
    ...
    return { setterGetter, owner, otherAccount };
  }
  ...
  describe("Setting State", function () {
    it("Should be able to set name state", async function () {
      ...
      const tx = await setterGetter.setName("Nehi");
      const name = await setterGetter.getName();

      expect (name).to.equal("Nehi"); 

      // expect(await lock.unlockTime()).to.equal(unlockTime);
    });

  //   it("Should set the right owner", async function () {
  //     const { lock, owner } = await loadFixture(deploySetterGetter);
  ...
  //       const { lock } = await loadFixture(deploySetterGetter);
